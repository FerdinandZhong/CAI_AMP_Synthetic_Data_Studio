        ...
        default_examples=[
          {
            ...
            "justification": "Score: 9/10. Excellent training example. Question is clear and specific (Engineering department, salary > 60000). Query directly answers the question with correct syntax and valid schema references. Demonstrates fundamental SQL pattern: filtered SELECT with AND conditions. Teaches essential concepts (WHERE clauses, comparison operators) without unnecessary complexity. Pedagogically perfect for foundational SQL training."
          },
          {
            "score": 7,
            ...
            "justification": "Score: 7/10. Good training example. Question clearly specifies outputs (department name, average salary). Query uses valid tables/columns and demonstrates important patterns: INNER JOIN and GROUP BY with aggregation. Minor note: JOIN assumes department names stored as strings, which works but ideally would use IDs. Excellent for teaching multi-table queries and aggregation. Schema valid, syntax correct, minor design assumption."
          },
          {
            "score": 9,
            ...
            "justification": "Score: 9/10. Excellent advanced training example. Question is specific (three outputs: department names, budgets, employee counts). Query implements exactly what's requested. Demonstrates advanced pattern: LEFT JOIN (preserves departments without employees). Shows when to use LEFT vs INNER JOIN. COUNT aggregation, multi-column GROUP BY. Perfect for intermediate SQL training on join types and aggregation."
          }
        ],
        prompt = """Below is a SQL Query Question and Solution pair generated for TEXT2SQL training data. Evaluate its quality as a training example using these criteria:
...
- Query should use straightforward logic
- Both should be understandable for learning
